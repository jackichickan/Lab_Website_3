...
app.get('/player_info/select_player', function(req, res) {
  ...
  var query3 = "SELECT COUNT(*) as games_played FROM football_players INNER JOIN football_games ON football_players.id = ANY(football_games.players) WHERE football_players.id = '"+player_id+"';";
  db.task('get-everything', task => {
      return task.batch([
        task.any(query1),
        task.any(query2),
    ...
    .then(data => {
      res.render('pages/player_info', {
        my_title: "Player Info",
        all_football_players: data[0],
        my_player_data: data[1],
        my_player_games: data[2]
      })
    })
    .catch(err => {
      // display error message in case an error
